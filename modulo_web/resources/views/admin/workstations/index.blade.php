    ...
    <x-card>
        <div class="index-toolbar">
            <form method="GET" action="{{ route('admin.workstations.index') }}" class="search-form">
                ...
                'col') === '' ? 'selected' : '' }}>Todos os campos
                    ...
                    <option value="desc" {{ request(
                'col') === 'desc' ? 'selected' : '' }}>Descrição
                    </option>
                    <option value="hash" {{ request(
                'col') === 'hash' ? 'selected' : '' }}>Hash
                    </option>
                </select>
        ...
        <x-table :headers="['Hash', 'Descrição', 'Ações']">
            ...
            @if($workstations->isEmpty())
                <tr>
                    <td colspan="3" style="text-align: center; color: var(--secondary);">Nenhuma estação encontrada.
                    </td>
                </tr>
            @endif
        </x-table>
