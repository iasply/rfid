        ...
        <form method="GET" action="{{ route('admin.alerts') }}" class="search-form">
            <select name="type" class="col-select" onchange="this.form.submit()">
                ...
                @foreach($vaccineTypes as $type)
                    <option value="{{ $type }}" {{ $typeFilter=== $type ?
                ...
                <div
                    ...
                    <div>
                        <h3 style="margin: 0 0 0.35rem; font-size: 1.05rem; font-weight: 800; color: var(--text-main);">
                            ...
                            @if($group['in_season'])
                                <span
                                    style="font-size: 0.7rem; font-weight: 700; background: rgba(16,185,129,0.12); color: var(--primary-dark); padding: 0.15rem 0.55rem; border-radius: 999px; vertical-align: middle; margin-left: 0.4rem;">em época</span>
                            @endif
                        </h3>
                    ...
                    <div style="display: flex; gap: 0.4rem; flex-wrap: wrap; flex-shrink: 0;">
                        @php
                            ...
                            $tOverdue = $group['total'] - collect($group['rows'])->where('urgency', 'due_soon')->count() -
                            collect($group['rows'])->where('urgency', 'never')->count();
                        @endphp
                ...
                <table class="activity-table">
                    ...
                    @foreach($group['rows'] as $row)
                        <tr style="{{ $row['urgency'] === 'overdue' ? 'background: rgba(239,68,68,0.03);' : '' }}">
                            ...
                            <td>
                                @if($row['last_vax'])
                                    {{ Carbon::parse($row['last_vax'])->format('d/m/Y') }}
                            ...
                            <td>
                                @if($row['next_due'])
                                    ...
                                    @if($row['days_remaining'] <= 0)
                                        <span
                                            style="display: block; font-size: 0.74rem; color: var(--danger); font-weight: 700;">
                                            {{ abs($row['days_remaining']) }}d em atraso
                                        </span>
                                    @else
                                        <span
                                            style="display: block; font-size: 0.74rem; color: #92400e; font-weight: 600;">
        ...
        <x-card>
            <div style="text-align: center; padding: 3rem; color: var(--text-muted);">
                @if(!$typeFilter)
                    ...
                    <h3 style="margin: 0 0 0.5rem; font-size: 1.125rem; color: var(--text-main);">Selecione um tipo de
                        vacina</h3>
                    <p style="margin: 0; font-size: 0.9rem;">Escolha um tipo de vacina no filtro acima para ver os
                        avisos de vacinação do rebanho.</p>
                @else
                    <div style="font-size: 3rem; margin-bottom: 1rem;">✅</div>
                    <h3 style="margin: 0 0 0.5rem; font-size: 1.125rem; color: var(--text-main);">Rebanho em dia!</h3>
                    <p style="margin: 0; font-size: 0.9rem;">Nenhuma vacina pendente ou vencendo nos próximos 30
                        dias.</p>
                @endif
            </div>
        </x-card>
